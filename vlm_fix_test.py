...
"""
VLM 修复测试脚本
测试 create_with_multiple_images 方法是否支持 messages 和 image_sources 参数
"""
...
def main():
    ...
    print(f"[测试] 测试消息: {messages}")
    print(f"[测试] 测试图像源: {image_paths}")
    
    try:
        response = llm_service.create_with_multiple_images(messages, image_sources=image_paths)
        print("[测试] 多图分析响应:")
        print(response)
        print("[测试] VLM 多图分析修复测试成功!")
    except Exception as e:
        print(f"[测试] VLM 多图分析修复测试失败: {e}")
        import traceback
        traceback.print_exc()
